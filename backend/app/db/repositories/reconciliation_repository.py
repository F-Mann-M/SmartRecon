...
def _normalize_vendor_tokens(vendor_name: str) -> set[str]:
    """
    Normalizes vendor names by lowercasing, removing punctuation, 
    and filtering out common legal entity tokens.
    """
     # remove punctuation and special characters
    cleaned = re.sub(r"[^a-z0-9\s]", " ", vendor_name.lower())
    # split the cleaned string into tokens and filter out legal entity tokens
    return {
        token
...
async def get_date_similarity(invoice_date, transaction_date) -> bool:
    """compares the transaction date with the invoice date, allowing for a window of 5 days before and 20 days after the invoice date."""
    min_date = invoice_date - timedelta(days=5)
    max_date = invoice_date + timedelta(days=20)
    return min_date <= transaction_date <= max_date


async def find_best_invoice_match(db: AsyncSession, transactions: TransactionModel) -> Tuple[Optional[InvoiceModel], float, str]:
    """
    ...
    try:
        ...
        for transaction in transactions:
            ...
            exact_matches = (await db.scalars(select(InvoiceModel).where(InvoiceModel.total_amount == (transaction_amount * -1)))).all()
            ...
            for match in exact_matches:
                ...
                is_with_in_windows = await get_date_similarity(match.invoice_date, transaction.transaction_date)
        ...
        for bank_transaction, match in transaction_matches:
            ...
            if await is_match_already_reconciled(db=db, transaction_id=bank_transaction.id, invoice_id=match.id):
            ...
            await add_matches_to_reconciliation_table(db=db, transaction=bank_transaction, invoice=match, match_score=1.0, match_type="EXACT_MATCH")
...
async def add_matches_to_reconciliation_table(db: AsyncSession, transaction: TransactionModel, invoice: InvoiceModel, match_score: float, match_type: str) -> None:
    """
...
async def is_match_already_reconciled(db: AsyncSession, transaction_id: str, invoice_id: str) -> bool:
    """
    Checks if a given transaction and invoice pair is already reconciled.
    """
    result = select(
        exists().where(
            ReconciliationModel.transaction_id == transaction_id,
            ReconciliationModel.invoice_id == invoice_id
        )
    )    
    return await db.execute(result)


async def get_all_reconciliation_entries(db: AsyncSession) -> List[ReconciliationModel]:
    return (await db.scalars(select(ReconciliationModel))).all()
