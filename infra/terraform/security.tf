...
# NSG: Endpoints Subnet (Foundry, Storage, Database Private Endpoints)
resource "azurerm_network_security_group" "nsg_endpoints" {
  ...
  security_rule {
    ...
    destination_address_prefix = "*"
    description                = "Allow PostgreSQL traffic only from compute subnet"
  }

  security_rule {
    name                       = "Deny-Postgres-From-Rest-of-VNet"
    priority                   = 150
    direction                  = "Inbound"
    access                     = "Deny"
    protocol                   = "Tcp"
    ...
    source_address_prefix      = "VirtualNetwork"
    destination_address_prefix = "*"
    description                = "Deny PostgreSQL traffic from the rest of the VNet"
  }
